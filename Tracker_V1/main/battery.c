...
int adc_read_battery_voltage()
{
    ...
    if (calibration_enabled)
    {
        ...
        voltage_mv = ((voltage_mv > V_MAX) ? V_MAX : (voltage_mv < V_MIN) ? V_MIN : voltage_mv);
    }
    int bat = ((voltage_mv - V_MIN) * 100 / (V_MAX - V_MIN));

    int temp = (prev_bat + bat) / 2;
    prev_bat = bat;
    return temp;
}