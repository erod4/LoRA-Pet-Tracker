...
export default function LightSliderButton({
    ...
    const panResponder = useRef(
        PanResponder.create({
            ...
            onPanResponderMove: (_, { dx }) => {
                ...
                if (newPct % 5 === 0 && lastWarnedRef.current !== newPct) {
    ...
    return (
        ...
        >
            ...
            {/* padded content */}
            <View style={styles.content} className="p-3">
                <View className='rounded-full p-2 bg-[#ddd] w-8 h-8 items-center justify-center'>
                    <FontAwesomeIcon icon={faLightbulb} />
                </View>
                <Text className='font-bold color-white'>{isOn?`${brightness}% Brightness`:"Light Off"}</Text>
